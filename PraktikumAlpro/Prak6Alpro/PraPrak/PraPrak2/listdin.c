...
/* ********** KONSTRUKTOR ********** */
 ...
 void readList(ListDin *l){
    int N;
    do{scanf("%d",&N);}while(N<0 || N>l->capacity);
    l->nEff=N;
 ...
 ListDin plusMinusList(ListDin l1, ListDin l2, boolean plus){
    ...
    L.nEff=l1.nEff;
    if(plus){for(int i=0;i<l1.nEff;i++){
       L.buffer[i]=l1.buffer[i]+l2.buffer[i];
    }}else{
 ...
 void expandList(ListDin *l, int num){
    l->buffer = (ElType *)realloc(l->buffer, (l->capacity+num) * sizeof(ElType));
    l->capacity += num;
 };
